...
FormalPowerSeries<mint> all_prod(vc<FormalPowerSeries<mint>> v){
    ...
    priority_queue<poly,vc<poly>,decltype(cmp)>PriorityQueue(cmp);
    for(auto&p:v)PriorityQueue.push(std::move(p));
    while(PriorityQueue.size()>=2){
        auto p1=std::move(PriorityQueue.top());PriorityQueue.pop();
        auto p2=std::move(PriorityQueue.top());PriorityQueue.pop();
        PriorityQueue.push(std::move(p1*p2));
    }
