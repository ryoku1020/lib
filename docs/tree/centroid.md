...
頂点数を $N$ とします。コールバック本体の処理を除く分解処理は `O(N log N)` 時間、`O(N)` 補助領域です。

各コールバックで渡された `send` の連結成分を線形走査する場合も、同じ頂点が現れる回数は `O(log N)` なので、全コールバックを合わせて `O(N log N)` 時間です。コールバックがそれ以上の処理を行う場合は、その総時間を加えてください。内部 DFS は再帰実装のため、最悪 `O(N)` の再帰スタックを使います。
...
- `N==0` ではコールバックを 1 度も呼ばずに終了します。`N==1` ではその頂点について `callnonvirtual` が 1 回呼ばれます。
- `vvc<int>` 版の `g` は処理中に一時的に書き換えられますが、分解終了時に隣接リストは復元されます。処理中に別スレッドなどから参照しないでください。
- `send`, `col`, `is_virtual` の内容はコールバック後にクリアまたは更新されます。参照を保存せず、必要な情報はコールバック内でコピーしてください。
