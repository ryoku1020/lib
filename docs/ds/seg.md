...
- `e`
  単位元
- `leaf`
  `SegmentTree(int n)` で各葉をどう初期化するか
...
### `template<class F> SegmentTree(int n, F f)`
...
- `SegmentTree(int n)` は `leaf()` を使い、`prod` の単位元には `e()` を使います。ここを分けられるのが ACL と少し違う点です。
