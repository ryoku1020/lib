...
documentation_of: ../../ds/lazy_seg.hpp
---
...
- `e`
  `op` の単位元です。
- `leaf`
  `LazySegmentTree(int N)` で葉を何で初期化するかを指定したいときに定義します。
  未定義なら `e()` が使われます。
...
### `LazySegmentTree(int N, vc<value_type> A)`
...
### `template<class F> LazySegmentTree(int N, F f)`
