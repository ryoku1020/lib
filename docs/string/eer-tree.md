...
文字列長を `n` とすると、構築時間は `O(n)`、メモリは `O(sigma*n)` です。
異なる回文は高々 `n` 個で、各ノードが長さ `sigma` の遷移配列を持ちます。

## 境界・注意

- 空文字列も構築でき、2 個の特殊ノードだけが作られます。
- 全ての文字で `0 <= s[i]-CHRMIN < sigma` が必要です（`assert` あり）。
- `child[v][c]` の「存在しない」状態は `-1e9`（整数として大きな負の値）です。
  存在判定は `child[v][c] >= 0` で行えます。
- `sigma` は実際に使う文字数に合わせて設定してください。大きすぎるとメモリを消費します。
- `build` は状態を初期化せず、ノード番号に `static int id` を使います。同じ `sigma` について、1 プロセスで 1 個のインスタンスに 1 回だけ `build` する前提です。複数インスタンスや再構築は範囲外参照を起こし得ます。
